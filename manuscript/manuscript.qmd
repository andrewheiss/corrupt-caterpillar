...
{{< epigraph "Scientia et potentia humana in idem coincidunt, quia ignoratio causae destituit effectum. *(Human knowledge and human power meet in one; for where the cause is not known the effect cannot be produced.)*" source="Francis Bacon, *Novum Organum*, part I, aphorism III" >}}

{{< epigraph "Don't let us forget that the causes of human actions are usually immeasurably more complex and varied than our subsequent explanations of them." source="Fyodor Dostoevsky, *The Idiot*, part IV, chapter 2" >}}
...
Qualitative case studies: Rohlfing:2012, GeorgeBennett:2005, BennettCheckel:2015
...
From thread by Stephen Wild (https://bsky.app/profile/stephenjwild.bsky.social/post/3ldhouhkcsk2z)

- Causal inference is hard: https://www.nature.com/articles/s41562-024-01939-z
- Always clearly define your estimand: https://journals.sagepub.com/doi/10.1177/00031224211004187
- Threats to validity (with DAGs): https://journals.lww.com/epidem/fulltext/2020/05000/A_Graphical_Catalog_of_Threats_to_Validity_.11.aspx
- Crash course in good and bad controls: https://journals.sagepub.com/doi/full/10.1177/00491241221099552
- Need to think about causality when selecting control variables: https://journals.sagepub.com/doi/full/10.1177/25152459221095823
- Conditioning on colliders is generally bad: https://www.annualreviews.org/content/journals/10.1146/annurev-soc-071913-043455
...
ggplot(data_confounder_example, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  scale_color_manual(values = c("X" = clrs$Prism[4], "Y" = clrs$Prism[2], "Z" = clrs$Prism[6]), guide = "none") +
...
data_confounder <- dagify(
  ...
  tidy_dagitty() |> 
  mutate(arrow_color = ifelse(name == "Z", clrs$Prism[7], "grey80")) |> 
...
confounder <- ggplot(data_confounder, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  scale_color_manual(values = c("grey80", clrs$Prism[7]), guide = "none") +
  coord_cartesian(ylim = c(0.95, 2.05)) +
  labs(title = "Confounder") +
  theme_np_dag() +
  theme(plot.title = element_text(hjust = 0.5, color = clrs$Prism[7]))
...
data_mediator <- dagify(
  ...
  mutate(arrow_color = case_when(
    name == "Z" & to == "Y" ~ clrs$Prism[1], 
    name == "X" & to == "Z" ~ clrs$Prism[1],
    TRUE ~ "grey80")) |> 
  mutate(node_color = ifelse(name == "Z", TRUE, FALSE)) 
...
mediator <- ggplot(data_mediator, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  scale_color_manual(values = c("grey80", clrs$Prism[1]), guide = "none") +
  coord_cartesian(ylim = c(0.95, 2.05)) +
  labs(title = "Mediator") +
  theme_np_dag() +
  theme(plot.title = element_text(hjust = 0.5, color = clrs$Prism[1]))
...
data_collider <- dagify(
  ...
  tidy_dagitty() |> 
  mutate(arrow_color = ifelse(to == "Z", clrs$Prism[8], "grey80")) |> 
  mutate(node_color = ifelse(name == "Z", TRUE, FALSE)) 
...
collider <- ggplot(data_collider, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  scale_color_manual(values = c("grey80", clrs$Prism[8]), guide = "none") +
  coord_cartesian(ylim = c(0.95, 2.05)) +
  labs(title = "Collider") +
  theme_np_dag() +
  theme(plot.title = element_text(hjust = 0.5, color = clrs$Prism[8]))
...
For example, in 2022, using a large national-level observational dataset, researchers found an association between smoking behavior and reduced COVID-19 risk: people who smoked regularly saw a sigificantly reduction in the probability of contracting severe COVID-19. However, this paradoxical finding can be explained with collider bias [@AuYeungKwok:2022]. The dataset contained information about hospital patients who were (1) smokers diagnosed with COVID, (2) smokers diagnosed with smoking-related respiratory disease, and (3) non-smokers diagnosed with COVID. The data did not contain any patients who were non-smokers diagnosed with non-COVID, smoking-related respiratory disease, as these people were not patients and needed no medical treatment. In DAG terms, presence at the hospital was a collider (Smoking → Hospital ← Severe COVID), and conditioning the estimate of Smoking → Severe COVID on being a patient therefore distorted and biased the measured effect.
...
data_rct <- dagify(
  ...
  tidy_dagitty() |> 
  node_status() |> 
  mutate(arrow_color = ifelse(name == "X", clrs$Prism[8], "grey80"))
...
rct <- ggplot(data_rct, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  scale_color_manual(values = c(clrs$Prism[4], clrs$Prism[2]), 
                     na.value = clrs$Prism[6], guide = "none") +
  coord_cartesian(ylim = c(0.95, 2.05)) +
...
iv <- ggplot(data_iv, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  geom_dag_text(size = 4) +
  scale_color_manual(values = c(clrs$Prism[4], "grey80", clrs$Prism[2]), 
                     na.value = clrs$Prism[6], guide = "none") +
